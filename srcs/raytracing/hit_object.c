...
static t_ray (*const	g_get_normal_of_object[3])(void *object, t_vec3 contact) = {
...
static t_bool	set_hit(t_hit *hit, t_object *object, double dist, t_ray ray)
{
	...
	hit->ray.direction = vec3_mul(ray.direction, dist / vec3_length(ray.direction));
	hit->normal = (t_ray){(t_vec3){0, 0, 0}, (t_vec3){0, 0, 0}};
	hit->color = (t_vec3){0, 0, 0};
	if (dist < INF)
	{
		hit->normal = g_get_normal_of_object[hit->object->shape](
				hit->object->object,

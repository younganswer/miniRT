...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/25 14:56:27 by younhwan          #+#    #+#             */
/*   Updated: 2023/02/25 14:56:28 by younhwan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	parse_plane(t_var *var, char **splited)
{
	...
	if (parse_vec3(var, &plane->point, splited[0]) == FALSE || \
		parse_vec3(var, &plane->normal, splited[1]) == FALSE || \
		dir_range_is_valid(var, plane->normal) == FALSE || \
		parse_vec3(var, &plane->color, splited[2]) == FALSE || \
		color_range_is_valid(var, plane->color) == FALSE || \
		parse_type(var, &plane->type, splited[3]) == FALSE)
		return (set_err(var, INVALID_ARG) && FALSE);
